...
@laslogin_required
@login_required
@permission_decorator('sangerApp.can_view_SAM_upload_request')
@transaction.commit_manually
def pending_request(request):
    print 'SAM start view: views_request.pending_request'
    if request.method == "GET":
        # GET method
        try:
            user = auth.get_user(request)
            user_name = User.objects.get(username = user.username)
            plans = Request.objects.filter(pending=True, timechecked__isnull =True,  time_executed__isnull =True)
            transaction.commit()
        ...
        try:
            ...
            print '[SAM] - Raw Data: %s' % raw_data['idplan']
            return HttpResponseRedirect(reverse('sangerApp.views.upload_request', kwargs={'request_id':raw_data['idplan']}) )
        except Exception, e:
            print e
            return HttpResponseBadRequest("Page not available")
...
def upload_request(request, request_id):
    print 'SAM start view: views_request.upload_request'
    resp ={}
    if request.method == "GET":
        # GET method
        try:
            ...
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Page not available! " + str(e.args[0]))
        finally:
            transaction.rollback()
...
def confirm_request(request):
    print 'SAM start view: views_request.confirm_request'
    if request.method == "POST":
        try:
            ...
            transaction.commit()
            return HttpResponseRedirect(reverse('sangerApp.views.create_request'))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Error in saving data")
...
def create_request(request):
    print 'SAM start view: views_request.create_request'
    resp = {}
    if request.method == "GET":
        try:

 ...
 * Created:     Tue Oct 15 00:00:00 BST 2013
 ...
 * Contact:     
 * 
 * Copyright 2013 Alessandro Fiori, all rights reserved.
 *
...
/** 
 * HVis provides column visiblity control for DataTables
 * @class HVis
 * @constructor
 * @param {object} DataTables settings object
 */
HVis = function( oDTSettings, oInit )
{
	/* Santiy check that we are a new instance */
	if ( !this.CLASS || this.CLASS != "HVis" )
	{
		alert( "Warning: HVis must be initialised with the keyword 'new'" );
	}
	...
	/**
	 * @namespace Settings object which contains customisable information for HVis instance
	 */
	this.s = {
		/**
		...
		/**
		 * Indicate if HVis should automatically calculate the size of buttons or not. The default
...
HVis.prototype = {
	...
	"_fnConstruct": function ()
	{
		...
		this.dom.wrapper.className = "HVis TableTools";
		
		this.dom.button = this._fnDomBaseButton( this.s.buttonText );
		this.dom.button.className += " HVis_MasterButton";
		...
		/* Store the original selection information */
		for ( i=0, iLen=this.s.dt.aoColumns.length ; i<iLen ; i++ )
		{
			this.s.abOriginal.push( false );
		}
	 ...
	 * update the table's column visibility and HVis will still be accurate.
	...
	{
		console.log("_fnDrawCallback");
        var aoColumns = this.s.dt.aoColumns;
		for ( var i=0, iLen=aoColumns.length ; i<iLen ; i++ )
		{
			console.log("button: ", this.dom.buttons[i]);
            if ( this.dom.buttons[i] !== null )
			{
				console.log("vector: ", this.s.abOriginal[i]);
                if ( this.s.abOriginal[i] )
				{
	...
	"_fnAddButtons": function ()
	{
		...
		if ( this.s.bRestore )
		{
			...
			nButton.className += " HVis_Restore";
			this.dom.buttons.push( nButton );
			this.dom.collection.appendChild( nButton );
		}
		...
		if ( this.s.bShowAll )
		{
			...
			nButton.className += " HVis_ShowAll";
			this.dom.buttons.push( nButton );
			this.dom.collection.appendChild( nButton );
		}
	},
	...
	"_fnDomRestoreButton": function ()
	{
		...
		nButton.className = !this.s.dt.bJUI ? "HVis_Button TableTools_Button" :
			"HVis_Button TableTools_Button ui-button ui-state-default";
		nButton.appendChild( nSpan );
		$(nSpan).html( '<span class="HVis_title">'+this.s.sRestore+'</span>' );
		
		$(nButton).click( function (e) {
			that._fnSetSelection();
	...
	"_fnDomShowAllButton": function ()
	{
		...
		nButton.className = !this.s.dt.bJUI ? "HVis_Button TableTools_Button" :
			"HVis_Button TableTools_Button ui-button ui-state-default";
		nButton.appendChild( nSpan );
		$(nSpan).html( '<span class="HVis_title">'+this.s.sShowAll+'</span>' );
	...
	"_fnSetSelection": function(){
		var i, j, iLen, jLen, k;
		var aColumnsInc = this._fnColumnTargets( this.s.mColumns );
		for ( i=0, iLen=this.s.dt.aoData.length ; i<iLen ; i++ )
		{
			k = 0;
			for ( j=0, jLen=this.s.dt.aoColumns.length ; j<jLen ; j++ ){
				if ( aColumnsInc[j] ){
					if (this.s.abOriginal[j]){
						$(this.s.dt.aoData[i].nTr.cells[k]).addClass('highsel');
					}
					else {
						$(this.s.dt.aoData[i].nTr.cells[k]).removeClass('highsel');
					}
				k++;
				}
			}
	...
	"_fnDomColumnButton": function ( i )
	{
		...
		nButton.className = !dt.bJUI ? "HVis_Button TableTools_Button" :
			"HVis_Button TableTools_Button ui-button ui-state-default";
		nButton.appendChild( nSpan );
		var sTitle = this.s.fnLabel===null ? oColumn.sTitle : this.s.fnLabel( i, oColumn.sTitle, oColumn.nTh );
		$(nSpan).html(
			'<span class="HVis_radio"><input type="checkbox"/></span>'+
			'<span class="HVis_title">'+sTitle+'</span>' );
		$(nButton).click( function (e) {
	...
	/**
	 * Get the position in the DataTables instance array of the table for this instance of HVis
	...
	"_fnDomCollection": function ()
	{
		...
		nHidden.className = !this.s.dt.bJUI ? "HVis_collection TableTools_collection" :
			"HVis_collection TableTools_collection ui-buttonset ui-buttonset-multi";
	...
	"_fnDomCatcher": function ()
	{
		...
		nCatcher.className = "HVis_catcher TableTools_catcher";
	...
	"_fnDomBackground": function ()
	{
		...
		nBackground.className = "HVis_collectionBackground TableTools_collectionBackground";
	...
	"_fnCollectionShow": function ()
	{
		...
		for ( i=0, iLen=this.dom.buttons.length ; i<iLen ; i++ )
		{
			if ( this.dom.buttons[i] != null){
				if(aColumnsInc[i]){
					this.dom.buttons[i].style.display = "";
				}
				else{
					this.dom.buttons[i].style.display = "none";
				}
			}
		}
 ...
 *  @method  HVis.fnRebuild
 *  @static
 *  @param   object oTable DataTable instance to consider - optional
 *  @returns void
 */
HVis.fnRebuild = function ( oTable )
{
	var nTable = null;
	...
	for ( var i=0, iLen=HVis.aInstances.length ; i<iLen ; i++ )
	{
		if ( typeof oTable == 'undefined' || nTable == HVis.aInstances[i].s.dt.nTable )
		{
			HVis.aInstances[i].fnRebuild();
		}
	}
};
...
/**
 * Collection of all HVis instances
 *  @property HVis.aInstances
 *  @static
 *  @type     Array
 *  @default  []
 */
 ...
 */
HVis.VERSION = "1.0.8";
HVis.prototype.VERSION = HVis.VERSION;
     ...
     $.fn.dataTableExt.fnVersionCheck('1.7.0') )
{
	$.fn.dataTableExt.aoFeatures.push( {
		"fnInit": function( oDTSettings ) {
			var init = (typeof oDTSettings.oInit.oHVis == 'undefined') ?
				{} : oDTSettings.oInit.oHVis;
			var oHVis = new HVis( oDTSettings, init );
			return oHVis.dom.wrapper;
		},
		"cFeature": "H",
		"sFeature": "HVis"
	} );
...
{
	alert( "Warning: HVis requires DataTables 1.7 or greater - www.datatables.net/download");
}

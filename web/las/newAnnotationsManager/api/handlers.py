        ...
        try:
            ...
            if exactMatch.lower() in ['true', 'yes']:
                for geneSymbol in geneSymbolList:
                    ...
                    gene_ids2 = _GeneAlias.objects.values_list('id_gene', flat=True).filter(gene_synonym=geneSymbol).only('id_gene')
                ...
                for geneSymbol in geneSymbolList:
                    ...
                    gene_ids2 = _GeneAlias.objects.values_list('id_gene', flat=True).filter(gene_synonym__icontains=geneSymbol).only('id_gene')
                    gene_ids = list(chain(gene_ids1, gene_ids2))
                    gene_objs = Gene.objects.filter(id_gene__in=gene_ids)
                    genes[geneSymbol] = gene_objs
            else:
...
class Write_Annotation_SequenceAlteration(BaseHandler):
    allowed_method = ('POST')
    def create(self, request):
        try:
            id_sample = request.POST['id_sample']
            id_experimental_data = request.POST['id_experimental_data']
            id_analysis = request.POST['id_analysis']
        except:
            return ['Error', 'Invalid parameters']
        graph_uuid = request.POST.get('graph_uuid', None)
        if 'allele_frequency' in request.POST:
            try:
                allele_frequency = json.dumps(json.loads(request.POST['allele_frequency']))
            except:
                return ['Error', 'allele_frequency is not in JSON format']
        else:
            allele_frequency = None

        try:
            a = Annotation_SequenceAlteration()
            a.id_sample = id_sample
            a.id_experimental_data = id_experimental_data
            a.id_analysis = id_analysis
            a.graph_uuid = graph_uuid
            a.allele_frequency = allele_frequency
            a.save()
...
class Write_Annotation_CopyNumberVariation(BaseHandler):
    allowed_method = ('POST')
    def create(self, request):
        try:
            id_sample = request.POST['id_sample']
        ...
        graph_uuid = request.POST.get('graph_uuid', None)
        try:
            a = Annotation_CopyNumberVariation()
            a.id_sample = id_sample
            a.id_experimental_data = id_experimental_data
            a.id_analysis = id_analysis
            a.graph_uuid = graph_uuid
            a.copy_number = copy_number
            a.save()
...
class Write_Annotation_FusionTranscript(BaseHandler):
    allowed_method = ('POST')
    def create(self, request):
        try:
            id_sample = request.POST['id_sample']
            id_experimental_data = request.POST['id_experimental_data']
            id_analysis = request.POST['id_analysis']
            tx_5p_uuid = request.POST['tx_5p_uuid']
            tx_3p_uuid = request.POST['tx_3p_uuid']
            start_5p = request.POST['start_5p']
            end_5p = request.POST['end_5p']
            start_3p = request.POST['start_3p']
            end_3p = request.POST['end_3p']
        except:
            return ['Error', 'Invalid parameters']
        graph_uuid = request.POST.get('graph_uuid', None)
        try:
            a = Annotation_FusionTranscript()
            a.id_sample = id_sample
            a.id_experimental_data = id_experimental_data
            a.id_analysis = id_analysis
            a.graph_uuid = graph_uuid
            a.tx_5p_uuid = tx_5p_uuid
            a.tx_3p_uuid = tx_3p_uuid
            a.start_5p = start_5p
            a.end_5p = end_5p
            a.start_3p = start_3p
            a.end_3p = end_3p

...
def set_initWG(initWG):
    if settings.USE_GRAPH_DB == True:
        global _requests
        t = currentThread()
        if not t in _requests:
            raise GlobalRequestMiddlewareException
        _requests[t].initWG=initWG
    return
...
def get_initWG():
    if settings.USE_GRAPH_DB == True:
        ...
        if not t in _requests:
            ...
            x.user=''
            _requests[t]=x
            _requests[t].initWG=set()
        return _requests[t].initWG
    else:
        return set()
...
class GlobalRequestMiddleware(object):
    def process_request(self, request):
        try:
            ...
            t = currentThread()
            x= userInfo()
            x.user=request.user.username
            if 'HTTP_LASFUNCTIONALITY' in request.META:
                print request.META['HTTP_LASFUNCTIONALITY']
                try:
                    if request.META['HTTP_LASFUNCTIONALITY'] !='':
                        x.functionality = request.META['HTTP_LASFUNCTIONALITY']
                        user= User.objects.get(username=request.user.username)
                        wgSet = set(WG_User.objects.filter(user=user,permission=Permission.objects.get(codename=x.functionality)).values_list('WG__name',flat=True))
                        x.WG = wgSet
                except Exception,e:
                    print e

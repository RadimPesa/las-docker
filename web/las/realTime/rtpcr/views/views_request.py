...
@laslogin_required
@login_required
#@user_passes_test(lambda u: u.has_perm('rtpcr.can_view_RTM_upload_request'),login_url=reverse_lazy('rtpcr.views.error'))
@permission_decorator('rtpcr.can_view_RTM_upload_request')
...
def pending_request(request):
    if request.method == "GET":
        ...
        try:
            ...
            return HttpResponseRedirect(reverse('rtpcr.views.upload_request', kwargs={'request_id':raw_data['idplan']}) )
        except:
            return HttpResponseBadRequest("Page not available")
    ...
    if request.method == "GET":
        ...
        try:
            ...
            for g in get_WG():
                ...
                for u in User.objects.exclude(username='admin').exclude(first_name='').filter(id__in=WG_User.objects.filter(user__is_active=1,WG__name=g,permission__codename=get_functionality()).values_list("user",flat=True)).order_by('last_name'):
...
def confirm_request(request):
    if request.method == "POST":
        try:
            ...
            transaction.commit()
            return HttpResponseRedirect(reverse('rtpcr.views.create_request'))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Error in saving data")

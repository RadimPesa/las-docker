...
@laslogin_required
@login_required
@permission_decorator('rtpcr.can_view_RTM_validate_aliquots')
...
def select_request(request):
    if request.method == "GET":
        ...
        return HttpResponseRedirect(reverse('rtpcr.views.validate_samples'))
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if raw_data['status'] == 'continue':
                ...
                return HttpResponseRedirect(reverse('rtpcr.views.define_experiment'))
            elif raw_data['status'] == 'stop':
                # if the user stops at the virtual plan
                request.session['val_aliquots'] = raw_data['val_aliquots']
                request.session['plan'] = plan
                return HttpResponseRedirect(reverse('rtpcr.views.validated_request'))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Error")
...
@laslogin_required
@login_required
@permission_decorator('rtpcr.can_view_RTM_define_experiment')
@transaction.commit_manually
        ...
        try:
            ...
            for al in aliquots:
                ...
                for targetUUID in raw_data['targets']:
                    print samples[str(al.id)]
                    for x in range(samples[str(al.id)]):
                        sample = Sample(idAliquot_has_Request = al, probe = targetUUID, idExperiment=exp, time_creation=now)
                        sample.save()
                        print sample
                    summary = {'genid': al.aliquot_id.genId, 'replicates': samples[str(al.id)]}
            ...
            transaction.commit()
            return HttpResponseRedirect(reverse('rtpcr.views.layoutExperiment'))
        except Exception, e:
            print e
            transaction.rollback()

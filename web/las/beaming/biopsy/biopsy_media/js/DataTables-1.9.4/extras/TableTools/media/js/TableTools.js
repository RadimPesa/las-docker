 ...
 * Project:	    DataTables
 * Modified:    Tue Oct 15 00:00:00 BST 2013 by Alessandro Fiori
 * 
...
TableTools.prototype = {
	...
	"fnPdf": function ( oConfig )
	{
		if ( this.s.dt )
		{
			return this._fnPdf( oConfig );
		}
	},	

	"fnLAS": function ( oConfig ){
		if ( this.s.dt )
		{
			return this._fnLAS( oConfig );
		}
	},

	"fnDat": function ( oConfig ){
		if ( this.s.dt )
		{
			return this._fnDat( oConfig );
		}
	},

	"fnExcel": function ( oConfig ){
		if ( this.s.dt )
		{
			return this._fnExcel( oConfig );
		}
	},
	...
	"_fnCustomiseSettings": function ( oOpts )
	{
		...
		this.s.sUrl = this.s.custom.sUrl;
		/* Flash file location */
	...
	"_fnLAS": function ( oConfig ){
		var data = this._fnGetData(oConfig);
		data['fileformat'] = 'las'
		return JSON.stringify(data);
	},

	"_fnDat": function ( oConfig ){
		var data = this._fnGetData(oConfig);
		data['fileformat'] = 'data'
		return JSON.stringify(data);
	},

	"_fnExcel": function ( oConfig ){
		var data = this._fnGetData(oConfig);
		data['fileformat'] = 'excel'
		return JSON.stringify(data);
	},
	

	"_fnPdf": function ( oConfig ){
		var data = this._fnGetData(oConfig);
		data['fileformat'] = 'pdf'
		return JSON.stringify(data);
	},

	"_fnGetData": function ( oConfig ){
		var i, iLen, j, jLen, k;
		var dt = this.s.dt;
		...
		for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ ){
			if ( aColumnsInc[i] ){
				header.push({'title': this._fnHtmlDecode(dt.aoColumns[i].sTitle), 'class':dt.aoColumns[i].sClass});
			}
		}
		...
		for ( i=0, iLen=dt.aoData.length ; i<iLen ; i++ ){

			var row = [];
			k = 0;
			for ( j=0, jLen=dt.aoColumns.length ; j<jLen ; j++ ){
				if ( aColumnsInc[j] ){
					row.push({'data':this._fnHtmlDecode(dt.aoData[aDataIndex[i]]._aData[j]), 'class':dt.aoData[aDataIndex[i]].nTr.cells[k].className});
					k++;
				}
			}
			body.push(row);
		...
		return {'header':header, 'body':body, 'filename':oConfig.sTitle, 'title':oConfig.sPdfMessage};
...
TableTools.BUTTONS = {
	...
	"pdf": $.extend( {}, TableTools.buttonBase, {
		...
		"fnClick": function( nButton, oConfig) {
		        ...
		        nInput.setAttribute( 'name', 'tabledata' );
		        nInput.setAttribute( 'type', 'text' );
		        nInput.value = aoPost;
		         
		        nForm.appendChild( nInput );
		        
		        nForm.setAttribute( 'action', this.s.sUrl);
	...
	"las": $.extend( {}, TableTools.buttonBase, {
		"sAction": "text",
		"sButtonClass": "DTTT_button_las",
		"sButtonText": "LAS",
		"fnClick": function( nButton, oConfig) {
		        var oParams = this.s.dt.oApi._fnAjaxParameters( this.s.dt );
		        var aoPost = this.fnLAS( oConfig );
		        var aoGet = [];
	...
	"xls": $.extend( {}, TableTools.buttonBase, {
		...
		"fnClick": function( nButton, oConfig) {
		        var oParams = this.s.dt.oApi._fnAjaxParameters( this.s.dt );
		        var aoPost = this.fnExcel( oConfig );
		        var aoGet = [];
	...
	"data": $.extend( {}, TableTools.buttonBase, {
		"sAction": "text",
		"sButtonClass": "DTTT_button_data",
		"sButtonText": "Data",
		"fnClick": function( nButton, oConfig) {
		        var oParams = this.s.dt.oApi._fnAjaxParameters( this.s.dt );
		        var aoPost = this.fnDat( oConfig );
		        var aoGet = [];

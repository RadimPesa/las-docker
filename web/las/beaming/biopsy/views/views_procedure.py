...
@laslogin_required
@login_required
@permission_decorator('biopsy.can_view_LBM_validate_aliquots')
...
def select_request(request):
    ...
    if request.method == "GET":
        ...
        return HttpResponseRedirect(reverse('biopsy.views.validate_samples'))
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if raw_data['status'] == 'continue':
                ...
                return HttpResponseRedirect(reverse('biopsy.views.define_experiment'))
            elif raw_data['status'] == 'stop':
                # if the user stops at the virtual plan
                request.session['val_aliquots'] = raw_data['val_aliquots']
                request.session['plan'] = plan
                return HttpResponseRedirect(reverse('biopsy.views.validated_request'))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Error")
...
@laslogin_required
@login_required
@permission_decorator('biopsy.can_view_LBM_define_experiment')
@transaction.commit_manually
...
def define_experiment(request):
    if request.method == 'GET':
        try:
            ...
            for m in mut_used:
                m['genename'] = infoMut['genes'][m['gene']]
                m['mut_aa'] = infoMut['mutations'][m['probe']][0]
                m['mut_cds'] = infoMut['mutations'][m['probe']][1]
            print mut_used
            transaction.commit()
            return render_to_response('define_experiment.html', {'plan':plan, 'aliquots':aliquots, 'instruments':instruments, 'layouts':layouts, 'mutations':mut_used}, RequestContext(request))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Page not available")
        ...
        try:
            ...
            for s in raw_data['samples']:
                ...
                sample = Sample(idAliquot_has_Request = al, position=s['pos'], probe = s['probe'], gene=s['gene'], idExperiment=exp)
                sample.save()
                savedSamples.append({'genId': sample.idAliquot_has_Request.aliquot_id.genId, 'position':s['pos'], 'gene':s['genename'], 'mut':s['probe'], 'aa':s['aa'], 'cds':s['cds']})
            request.session['samples'] = savedSamples
            request.session['plan'] = exp
            transaction.commit()
            return HttpResponseRedirect(reverse('biopsy.views.layoutExperiment'))
        except Exception, e:
            print e
            transaction.rollback()

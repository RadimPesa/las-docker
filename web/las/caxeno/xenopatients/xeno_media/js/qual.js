...
jQuery(document).ready(function () {
    ...
    oTable = jQuery('#measure').dataTable( {
        ...
        "iDisplayLength": -1,
        "aoColumnDefs": [
            { "bVisible": false, "aTargets": [ 8,10,12,13,16 ] },
        ],
        "aaSorting": [[0, 'asc']]
    });
...
function wToB(evt){
    ...
    if ( charCode == 13 ) //codice ASCII del carattere carriage return (invio) || charCode == 9
        if ($('#id_barcode').val() == ''){
            $('#id_barcode').focus();
        }
        else{
...
function addRow(tableID, mouseGroup,status, mouse, groups_) {
    ...
    console.log(!isNaN(wCheck), wCheckFlag)
    if (!isNaN(wCheck) && wCheckFlag == true){
...
function addTreat(currG, barcode, nameP, nameA, date, duration, time, index){
    console.log(currG, barcode, nameP, nameA, date, duration, time, index);
    console.log(physBio[barcode])
    for (biomouse in physBio[barcode]){

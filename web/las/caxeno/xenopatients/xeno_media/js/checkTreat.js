    ...
    if (rows.length > 0){
        for(var i = 0; i < oTable.fnGetNodes().length; i++) {
            ...
            if (jQuery.inArray(row, rows) > -1){
			    ...
			    if (oTable.fnGetData(i, 9) != "" && oTable.fnGetData(i, 9) != "STOPPED"){
...
function writeTable(oTable, rows, nameP, nameA){
    comments = window.opener.jQuery('#checkNotes').val();
            ...
            for(var i = 0; i <= tot; i++) {
                ...
                if (jQuery.inArray(row, rows) > -1){
					...
					for (biomouse in window.opener.physBio[barcode]){
						...
						console.log(k);
						if (k < 0){
							window.opener.newTreat[genID]  = genID + '&' + nameT + '&' + start + '&' + comments;
						}else{
							console.log('else');
							var index = k;
                            var tr = window.opener.oSettings.aoData[index].nTr;
                            //var child = i + 1;
                            var genID = oTable.fnGetData(index,1);

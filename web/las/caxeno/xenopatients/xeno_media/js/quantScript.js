...
var nDecimal = 3; //variabile usata per chiamare la funzione trunc, inviandolo come valore del parametro decimal
physBio = {};
...
jQuery(document).ready(function () {
    ...
    oTable = jQuery('#measure').dataTable( {
        ...
        "iDisplayLength": -1,
        "aoColumnDefs": [
            { "bVisible": false, "aTargets": [ 2,3,4,7,8,13,15,16 ] },
        ],
        "aaSorting": [[0, 'asc']]
    });
...
function makeMeasure(){
    ...
    if (!isNaN(wCheck) && wCheckFlag == true){
        if (document.getElementById("id_barcode_of_mouse").value != ""){
            if ((!isNaN(xCheck)) && (!isNaN(yCheck)) && (!isNaN(wCheck)) ){
               if ( parseFloat(xCheck) > 0  && parseFloat(yCheck) > 0 && wCheckFlag == true){
                    jQuery.ajax({
                        url:url,
                        method:'get',
                        datatype:'json',
                    ...
                    alert("X, Y and weight values must be positive numbers." );
            }else
                alert("X, Y and weight values must be integer/decimal numbers.");
        }else{
...
function addRow(tableID, mouseGroup,status, mouse, xCheck, yCheck, zCheck, wCheck) {
    ...
    if (x > y )
        value = (y/2)*(y/2)*(x/2)*4/3*Math.PI;
    else
        value = (x/2)*(x/2)*(y/2)*4/3*Math.PI;

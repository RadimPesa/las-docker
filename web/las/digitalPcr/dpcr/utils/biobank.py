...
def retrieveAliquots (aliq_biobank, operator):
    print "[retrieveAliquots] aliq_biobank=%s, operator=%s" % (aliq_biobank, operator)
    ...
    print "[retrieveAliquots] res =", res
    return res
...
def setExperiment (aliquots):
    ...
    data = urllib.urlencode({'aliquots':aliquots, 'experiment':'DigitalPCR'})
...
def resetAliquotRequested (aliquots, operator):
    biobankUrl = Urls.objects.get(id_webservice=WebService.objects.get(name='biobank').id, available=True)
    aliquots = json.dumps(aliquots)
    data = urllib.urlencode({'aliquots':aliquots, 'experiment':'DigitalPCR', 'operator':operator})

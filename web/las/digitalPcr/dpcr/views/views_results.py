...
@laslogin_required
@login_required
@permission_decorator('dpcr.can_view_DPCR_filter_results')
...
def filter_results(request, filter_id):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            samples = Sample.objects.filter(idAliquot_has_Request__in=nsamples, wt=False)
            print samples
            ...
            #print measures
            resp['measuretype'] = MeasureType.objects.filter(pk__in=measures.values('idMeasureType'))
            transaction.commit()
        ...
        try:
            ...
            print samples
            print '---'
            measures = MeasurementEvent.objects.filter(idSample__in = samples.filter(gene__in = [p['gene'] for p in prunedPairs], probe__in =[p['probe'] for p in prunedPairs], wt = False).values('id'), idMeasureType__in = rawdata['measure']).order_by('idSample', 'idExperiment')
            data_resp = {}
            dictMeasure = {}
            dictMeasureBB = {}
            print measures
            ...
            formulaDict = getFormulasNameByIds(rawdata['formula'])
            print formulaDict
            ...
            transaction.commit()
            return HttpResponseRedirect(reverse('dpcr.views.view_results'))
        except Exception, e:
            print 'error' + str(e)
            transaction.rollback()
            return HttpResponseBadRequest("Page not available! " + e.args[0])

...
@laslogin_required
@login_required
@permission_decorator('dpcr.can_view_DPCR_upload_request')
...
def pending_request(request):
    if request.method == "GET":
        ...
        try:
            ...
            return HttpResponseRedirect(reverse('dpcr.views.upload_request', kwargs={'request_id':raw_data['idplan']}) ) 
        except:
            return HttpResponseBadRequest("Page not available")
...
def confirm_request(request):
    if request.method == "POST":
        try:
            ...
            transaction.commit()
            return HttpResponseRedirect(reverse('dpcr.views.create_request'))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Error in saving data")

...
function generate_table(){
    ...
    jQuery("#plan_table tbody td span.ui-icon-closethick").click(function(event) {
        ...
        var url =  base_url + '/api.deleteplan';
        jQuery.ajax({
            type: 'POST',
            url: url,
            async: false,
...
function unscheduleSession (){
    jQuery('#unschedule').click(function(event){
        var url =  base_url + '/select_plan/';
        var response = {'idplan': -1}
        var json_response = JSON.stringify(response);
        jQuery.ajax({
            type: 'POST',
            url: url,

...
function aggiorna_dati(){
	...
	if ((righesopra.length>1)||(listarighe.length>1)){
		...
		if((tipo=="position")||(tipo=="label")){
			listadisp=$("#aliq_originali tr").children(":nth-child(10)").not("th");
		}
		else if(tipo=="slide"){
...
function elimina_pianificazione(){
	...
	if (listaabort.length!=0){
		$("#conf,#id_barcode,#conf_all,#cancel").attr("disabled",true);
		$("#canc_sel").attr("disabled",false);
		if(tipo=="label"){
    		$("#no_validation").attr("disabled",true);
    	}
	}
	else{
		$("#conf,#id_barcode,#conf_all,#cancel").attr("disabled",false);
		$("#canc_sel").attr("disabled",true);
		if(tipo=="label"){
    		$("#no_validation").attr("disabled",false);
    	}
	}
	var selez=$(this).is(':checked');
	var genid=$(this).attr("gen");
	if(selez){		
		lista_canc.push(genid);
	}
	else{
...
function conf_barcode(){
	...
	if ($.inArray(barc,listaappoggio)!=-1){
		if ($.inArray(barc,listabarcgenerale)!=-1){
			...
			//tolgo 1 perche' l'ultima colonna e' il checkbox per la cancellazione
			var lunghezza = (oSettings.aoColumns.length-1);
			if(tipo=="slide"){
				//lo slide non ha l'ultima colonna con il check per la cancellazione
	    		lunghezza++;
	    	}
			var contat=0;
	        ...
	        jQuery.each(tabella.fnGetData(), function(key, d) {
	        	if ((d[7].toUpperCase() == barc)||(d[5].toUpperCase()== barc)||(d[3].toUpperCase()==barc)||(d[2].toUpperCase()==barc)){
	        		...
	        		if (dispon=="Yes"){
	        			var robot=$("#id_robot").val();
	        			//solo se sto rivalutando tramite robot
			    		if((robot=="True")&&(tipo=="revaluation")){
			    			var gen=d[1];
			    			if(gen in dizvolumi){
			    				var volumepresente=dizvolumi[gen];
			    				if(!volumepresente){
			    					alert("Aliquot "+gen+" is not liquid. You can't revalue it using robot");
			    					return;
			    				}
			    			}
			    		}
			...
			alert("You did not schedule to "+proc+" all children's container. Please validate barcode singularly.");
			$("#id_barcode").attr("value","");
			$("#id_barcode").focus();
		}
	}
...
$(document).ready(function () {
	...
	$("#conf_all").click(function(event){
		...
		if(vuota()){
	    	...
	    	if(tipo=="revaluation"){
	    		var url=base_url+"/revalue/execute/effective/";
	    	}
	    	else if(tipo=="split"){
	    		var url=base_url+"/split/execute/effective/";
	    	}
	    	else if(tipo=="position"){
	    		var url=base_url+"/vital/execute/effective/";
	    	}
	    	else if(tipo=="slide"){
	    		var url=base_url+"/slide/execute/effective/";
	    	}
	    	else if(tipo=="label"){
	    		var url=base_url+"/label/execute/effective/";
	    	}
			$.post(url, data, function (result) {
		    	if (result == "failure") {
		    		alert("Error");
		    		...
		    		var robot=$("#id_robot").val();
		    		if(robot=="True"){
		    			$("#form_conf").append("<input type='hidden' name='robot' />");
		    		}
		    		$("#form_conf").submit();
		    	}
			});
    ...
    $("#canc_sel").click(function(event){
    	...
    	if(tipo=="revaluation"){
    		var url=base_url+"/revalue/canc/";
    	}
    	else if(tipo=="split"){
    		var url=base_url+"/split/canc/";
    	}
    	else if(tipo=="position"){
    		var url=base_url+"/vital/canc/";
    	}
    	else if(tipo=="label"){
    		var url=base_url+"/label/canc/";
    	}
		$.post(url, data, function (result) {
	    	if (result == "failure") {
	    		alert("Error");

...
function aggiorna_Kit(){
	var prot=$("#id_protocol option:selected").attr("value");
	var urlprot=base_url+"/api/derived/"+prot;
	if(prot!=""){
		$.getJSON(urlprot,function(d){
			if(d.data!="errore"){
				stringa=d.data.name;
				$("#id_kit").attr("value",stringa);
				//$("#id_capacity,#id_single_kit").attr("value","");
				var urlpr=base_url+"/api/derived/calculate/"+prot;
				$.getJSON(urlpr,function(d){
					if(d.data!="errore"){
						...
						for(var j=0;j<righe.length;j++){
							...
							/*var rowPos=tab.fnAddData([$(righe[j]).children(":nth-child(1)").html(),$(righe[j]).children(":nth-child(2)").html(),
							               $(righe[j]).children(":nth-child(3)").html(),$(righe[j]).children(":nth-child(4)").html(),
							               $(righe[j]).children(":nth-child(5)").html(),$(righe[j]).children(":nth-child(6)").html(),
							               $(righe[j]).children(":nth-child(7)").html(),$(righe[j]).children(":nth-child(8)").html(),
							               $(righe[j]).children(":nth-child(9)").html()]);*/
							var rowPos=tab.fnAddData(listmp);
							var tableRowElement = tab.fnGetNodes(rowPos[0]);
							$(tableRowElement).attr("id",idriga);
							$(tableRowElement).children(":nth-child(13)").children(":nth-child(1)").click(seleziona_aliquote);
							$(tableRowElement).children(":nth-child(14)").children(":nth-child(1)").click(elimina_pianificazione);
							$(righe[j]).remove();
						...
						//prendo il valore del tipo di derivato per tutte le aliquote
						var lista=tab.$("tr").children(":nth-child(11)");
						var k=1;
						var nodes = tab.$("tr");
						var oid = [];
						var ii=0;
						...
						for(i=0;i<lista.length;i++){
							var der=$(lista[i]).text();
							//se il derivato non coincide rendo invisibile la riga
							if (der!=tipo){
								$(lista[i]).parent().hide();
								$("#temp").append($(lista[i]).parent());
								...
								var tipoaliqoriginale=$($(lista[i]).parent().children(":nth-child(9)").children()).val();
								trovato=0;
...
function seleziona_aliquote(){
	if(controlla()){
		...
		var lista=$(righetab).children(":nth-child(13)").children(":checked");
		var listaabort=$(righetab).children(":nth-child(14)").children(":nth-child(1)");
		//se non c'e' niente selezionato, riabilito protocollo e kit
		if (lista.length==0){
			$("#id_protocol").attr("disabled",false);
			$(listaabort).attr("disabled",false);
		}
		else{
			//disabilito la possibilita' di cambiare protocollo e kit
...
function elimina_pianificazione(){
	if(controlla()){
		...
		var listaabort=$(righetab).children(":nth-child(14)").children(":checked");
		var listaselect=$(righetab).children(":nth-child(13)").children(":nth-child(1)");
...
function selez_deselez(){
	if(controlla()){
		if ($(this).attr("sel")=="s"){
			...
			$("#select").text("");
			var lista=$(righetab).children(":nth-child(13)").children();
			...
			$.getJSON(urlst,function(d){
				if (d.data!="errore"){
					...
					if (contatore==lis_pezzi_url.length){
						...
						$(righetab).children(":nth-child(13)").children(":nth-child(1)").click(seleziona_aliquote);
						//metto l'evento click sull'abort
						$(righetab).children(":nth-child(14)").children(":nth-child(1)").click(elimina_pianificazione);
					}
...
$(document).ready(function () {
	...
	$("#id_protocol").change(aggiorna_Kit);
	...
	$("#conferma").click(function(event){
		...
		/*else if (righe.length==0){
			alert("Please select some aliquots to derive");
			event.preventDefault();
		}*/
		else{
			//faccio apparire l'alert all'utente se il singolo kit sta per finire.
			/*var capacit=$("#id_capacity").attr("value");
			if (($("#id_capacity").attr("value")!="")&&(righe.length)>capacit){
				alert("Kit selected can't do all this reactions. You have to open a new one.");
			}*/

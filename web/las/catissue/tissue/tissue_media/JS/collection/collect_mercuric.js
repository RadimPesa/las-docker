...
function controllacampi(place,dd,paz,barc,vol,indice){
...
function save() {
	...
	if(controllacampi(place,dd,paz,barc,vol,"")){
...
function addAliquot( barcodeT,file,diz){
    ...
    saveInLocal(sorgid, dat, paz, barcodeT, vol);
    contaaliq++;
}

function saveInLocal(sorgid, dat, paz, barcodeT, vol){
			...
			$.ajax({
			    ...
			    success: function(d) {
			        ...
			        for(var i=0;i<lista.length;i++){
			        	...
			        	if(!controllacampi(place,dd,paz,barc,vol,String(i+1))){
			        		return;
			        	}
...
$(document).ready(function () {
    ...
    $("#confirm_all").click(function(event){
    	...
    	if(!vuota()){
			...
			var url=base_url+"/mercuric/collection/save/";

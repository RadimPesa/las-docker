...
def set_ICCodeCollection(code):
    if settings.USE_GRAPH_DB == True:
        global _requests
        t = currentThread()
        if not t in _requests:
            raise GlobalRequestMiddlewareException
        _requests[t].codeUID=code
    return

def get_ICCodeCollection():
    if settings.USE_GRAPH_DB == True:
        global _requests
        t = currentThread()
        if not t in _requests:
            ...
            x.user=''
            _requests[t]=x
            _requests[t].codeUID=set()
        return _requests[t].codeUID
    else:
        return set()
...
class GlobalRequestMiddleware(object):
    def process_request(self, request):
        try:
            ...
            _requests[currentThread()].initWG=set()

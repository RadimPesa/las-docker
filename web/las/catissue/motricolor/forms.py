...
def carica_posti_collez(tipo):
    try:
        ...
        #lisfin.append(['1','Istituto di Candiolo'])
        #lisfin.append(['2','Mauriziano'])
        print 'lisfin',lisfin
        return lisfin
    except Exception,e:
        print 'err',e
...
class CollectionMotrForm(forms.Form):  
    def __init__(self, *args, **kwargs):
        self.type=kwargs.pop('tipo')
        super(CollectionMotrForm, self).__init__(*args, **kwargs)        
        print 'tipo',self.type
        lisposti=carica_posti_collez(self.type)
        ...
        self.fields['notes']=forms.CharField(label='Notes',max_length=100)

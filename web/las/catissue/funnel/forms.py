...
class CollectionFunnelForm(forms.Form):  
    def __init__(self, *args, **kwargs):
        super(CollectionFunnelForm, self).__init__(*args, **kwargs)
        tipi=(
            (0, 'Plasma'),
            (1, 'PBMC'),
        )
        lisposti=carica_posti_collez('Funnel')
        
        self.fields['Place'] = forms.ChoiceField(label='Site',choices=lisposti)
        self.fields['date']=forms.CharField()
        self.fields['patient']=forms.CharField(label='Funnel ID', max_length=30,required=False)
        self.fields['type']=forms.ChoiceField(choices=tipi,label='Type')
        self.fields['barcode']=forms.CharField(label='Tube barcode',max_length=30)
        self.fields['volume']=forms.CharField(label='Volume (ml)',max_length=8)

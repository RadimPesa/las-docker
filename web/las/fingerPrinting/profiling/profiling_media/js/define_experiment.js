...
function initSearchGene(){
    $("#search_gene").select2({
        width: 'resolve',
        placeholder: 'Start typing a SNP symbol...',
        ajax: {
            url: urlAnnot + "newapi/snpProbeInfo/",
            dataType: 'json',
            delay: 250,
            ...
            // alter the remote JSON data
                ...
                for (var i=0; i<data.length; i++){
                    if (!snpname.hasOwnProperty(data[i].name) ){
                        snpData.push({'id': data[i].name, 'symbol': data[i].name, 'chrom':data[i].chrom });
                        snpname[data[i].name] = true;
                    }
                }            
                return {results: snpData};
            },
        },
        ...
        templateResult: function(item) {
            ...
            return $('<b>'+item.symbol+'</b>&nbsp;<span class="small">(chrom: '+item.chrom +')</span>');
        },
...
function searchMut(geneId){
    ...
    var url =  urlAnnot + '/newapi/snpProbeInfo/';
    jQuery.ajax({
        type: 'GET',
        data: {'q': geneid},
        url: url,
        success: function(transport) {
            ...
            $(transport).each(function(index, value){
                console.log(value);
                jQuery('#table_mut').dataTable().fnAddData([value['uuid'], value['name'], value['allele'], value['alt'], value['class'], value['chrom'], value['start'], value['end'], value['strand']]);
...
function submit_data(){
    ...
    response = {'experiment': experiment_info, 'targets':target_to_send};
            ...
            success: function(transport) {
                ...
                for (var i=0; i< data['targets'].length; i++){
                    if (!targets_selected.hasOwnProperty(data['targets'][i]['uuid']) ){
                        targets_selected[data['targets'][i]['uuid']] = [data['targets'][i]['uuid'], data['targets'][i]['name'], data['targets'][i]['allele'], data['targets'][i]['alt'], data['targets'][i]['class'], data['targets'][i]['chrom'], data['targets'][i]['start'], data['targets'][i]['end'], data['targets'][i]['strand']];
                        jQuery("#table_experiment_targets").dataTable().fnAddData([null, data['targets'][i]['uuid'], data['targets'][i]['name'], data['targets'][i]['allele'], data['targets'][i]['alt'], data['targets'][i]['class'], data['targets'][i]['chrom'], data['targets'][i]['start'], data['targets'][i]['end'], data['targets'][i]['strand'] ])
                    }
                }
            }, 
            error: function(data) { 

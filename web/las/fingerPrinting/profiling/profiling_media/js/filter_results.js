...
function define_results_filter(){
    jQuery("#selected_regions").dataTable().fnClearTable();
    ...
    jQuery.each( jQuery("#region_selection").dataTable().fnGetNodes(), function(i, row){
        rowData = jQuery("#region_selection").dataTable().fnGetData(i);
        if (jQuery(row).find(':checkbox')[0].checked){
            jQuery("#selected_regions").dataTable().fnAddData([rowData[1], rowData[2]]);
            nreg++;
            list_regions.push(rowData[1]);
        }        
    });
    ...
    retrieveFormulas(list_regions, list_measures, genmeasures);
    jQuery('#div_selected_regions').show();
...
function define_formulas_filter(){
    ...
    var response = {'mask': [], 'target':[], 'formula':[], 'measure':[], 'region':[], 'bbmeasure':[]};
    var nequation = 0;
    ...
    jQuery.each( jQuery("#selected_regions").dataTable().fnGetNodes(), function(i, row){
        rowData = jQuery("#selected_regions").dataTable().fnGetData(i);
        response['region'].push(rowData[0]);        
    });
...
function retrieveFormulas(list_regions, list_measures, genmeasures){
    var url =  base_url + '/api.getFormulas';
    jQuery.ajax({
        type: 'GET',
        data: 'regions='+JSON.stringify(list_regions) + '&measures='+ JSON.stringify(list_measures) + '&general_measures=' + JSON.stringify(genmeasures),
        url: url,
...
// init the data table
function generate_selected_regions(){
    /*
     * Initialse DataTables, with image on the first column
     */
    jQuery("#selected_regions").dataTable( {
        "bProcessing": true,
         "aoColumns": [
            { "sTitle": "Key" },
            { "sTitle": "Region" },
    ...
    jQuery('#div_selected_regions').hide();
}

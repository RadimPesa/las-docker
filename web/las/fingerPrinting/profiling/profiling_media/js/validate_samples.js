...
jQuery(document).ready(function(){
    ...
    jQuery('#proceed,#save_stop').attr('disabled',true);
...
function submit_identifier(){
    var sample_identifier=jQuery('#alidentifier').val().toUpperCase();
    ...
    if ($.inArray(sample_identifier,listbarcode)!=-1){      
        ...
        //}
    	var table=$("#aliquot_toval_table").dataTable();
    	var aTrs = table.fnGetNodes();
    	jQuery.each(table.fnGetData(), function(key, d) {
    		
        	if ((d[7].toUpperCase() == sample_identifier)||(d[8].toUpperCase()== sample_identifier)){
        		aliquots_validated[d[7]] = aliquots_toval[d[7]];
        		
        		var rowtable=aTrs[key];
        		table.fnDeleteRow(rowtable);
        		
        		delete aliquots_toval[d[7]];
        		
        		// insert the new values
                aliquot_warnings = 0;
                jQuery('#aliquot_validated_table').dataTable().fnAddData([aliquots_validated[d[7]]['idaliquot'], aliquots_validated[d[7]]['genid'], aliquots_validated[d[7]]['sample_features'], aliquots_validated[d[7]]['owner'], aliquots_validated[d[7]]['volume'], aliquots_validated[d[7]]['concentration'], aliquots_validated[d[7]]['volumetaken'], null, aliquots_validated[d[7]]['barcode']]);
                if (Object.keys(aliquots_toval).length == 0){
                    jQuery('#proceed,#save_stop').removeAttr('disabled');
                }
        	}
    	});    	        
    }
...
function readTable(){
    ...
    jQuery.each(data, function(key, d) {
        ...
        //solo se il barcode non e' ancora in lista lo inserisce
        if (($.inArray(d[7].toUpperCase(),listbarcode)==-1)&&(d[7]!="None")){
        	listbarcode.push(d[7].toUpperCase());
        }
        //solo se la piastra non e' ancora in lista la inserisce
        if (($.inArray(d[8].toUpperCase(),listbarcode)==-1)&&(d[8]!="None")){
        	listbarcode.push(d[8].toUpperCase());
        }
    });
}

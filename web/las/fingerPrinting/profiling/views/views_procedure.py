...
@laslogin_required
@login_required
@permission_decorator('profiling.can_view_FPM_validate_aliquots')
...
def select_request(request):
    ...
    if request.method == "GET":
        ...
        return HttpResponseRedirect(reverse('profiling.views.validate_samples'))
    ...
    if request.method == "GET":
        ...
        try:
            ...
            if raw_data['status'] == 'continue':
                ...
                return HttpResponseRedirect(reverse('profiling.views.define_experiment'))
            elif raw_data['status'] == 'stop':
                # if the user stops at the virtual plan
                request.session['val_aliquots'] = raw_data['val_aliquots']
                request.session['plan'] = plan
                return HttpResponseRedirect(reverse('profiling.views.validated_request'))
        except Exception, e:
            print e
            transaction.rollback()
            return HttpResponseBadRequest("Error")
...
@laslogin_required
@login_required
@permission_decorator('profiling.can_view_FPM_define_experiment')
@transaction.commit_manually
...
def define_experiment(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            transaction.commit()
            return HttpResponseRedirect(reverse('profiling.views.layoutExperiment'))
        except Exception, e:
            print e
            transaction.rollback()

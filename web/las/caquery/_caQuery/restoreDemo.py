...
def find_parent_thru_transf_event(a_genid,a_values):
    return current_aliquots[a_genid][1]
    '''
    genid = GenealogyID(a_genid).getGenID()
    values_to_send = {'template_id':50, 'parameters':json.dumps([{'id':'0', 'values':[genid]}])} # change
...
def find_src_aliquot_thru_split(a_genid, a_values):
    return current_aliquots[a_genid][2]
    '''
    genid = GenealogyID(a_genid).getGenID()
    values_to_send = {'template_id':51, 'parameters':json.dumps([{'id':'0', 'values':[genid]}])} # change
    url = settings.DOMAIN_URL +  '/mdam/api/runtemplate/'
    data = urllib.urlencode(values_to_send)
...
def find_parent_thru_explant(a_genid):
    return current_aliquots[a_genid][0]
    '''
    genid = GenealogyID(a_genid).getGenID()
    values_to_send = {'template_id':52, 'parameters':json.dumps([{'id':'0', 'values':[genid]}])} # change
    url = settings.DOMAIN_URL +  '/mdam/api/runtemplate/'
    data = urllib.urlencode(values_to_send)
...
def find_parent_thru_implant(m_genid, m_values):
    return current_mice[m_genid][0]
    '''
    genid = GenealogyID(m_genid).getGenID()
    values_to_send = {'template_id':53, 'parameters':json.dumps([{'id':'0', 'values':[genid]}])} #change
    url = settings.DOMAIN_URL +  '/mdam/api/runtemplate/'
    data = urllib.urlencode(values_to_send)
...
def getCases():
    ...
    for c in result['body']:
        if c[7] != 'None': #'Collection Date'
            cases[c[0]] = str(datetime.datetime.strptime(c[7], '%Y-%m-%d')) # c[0] = 'Genealogy (7 cifre)', c[1] = 'collection date' 
        else:
...
def getAliquots(case):
    ...
    values_to_send = {'template_id':19, 'parameters':json.dumps([{'id':'0', 'values':[genidCase]}])} # CHANGE TEMPLATE ID
    url = settings.DOMAIN_URL +  '/mdam/api/runtemplate/'
    data = urllib.urlencode(values_to_send)
    ...
    current_aliquots.clear()

    for x in result['body']:
        g = GenealogyID(x[1]) # x[1] genid
        altype = 'c'+g.get2Derivation() if g.is2Derivation() else g.getArchivedMaterial()
        #print altype
        dateAl = datetime.datetime.strptime(x[3], '%Y-%m-%d') #  x[3] 'Sampling Date'
        print g.getGenID(), dateAl
        nodes[g.getGenID()] = {'type':'Aliquot', 'altype': altype, 'id':x[7], 'relationships':{}, 'nodeid': None, 'date':str(dateAl)} # x[0]= idaliquot
        buildSemanticNode(g.getGenID(), 'Aliquot', None)

        # x[8] = array of translator results (i.e. last element of the row)
        # x[8][0] = array of results for translator "Explants"
        # x[8][1] = array of results for translator "Mother aliquots derivation"
        # x[8][2] = array of results for translator "Mother aliquots split"
        # translator results can either include 0 rows or 1 row
        current_aliquots[x[1]] = [  x[8][0][0][1] if len(x[8][0]) > 0 else None, # field #2 = Father Mouse GenID
                                    x[8][1][0][0] if len(x[8][1]) > 0 else None, # field #0 = Mother Aliq. GenID
                                    x[8][2][0][0] if len(x[8][2]) > 0 else None] # field #0 = Mother Aliq. GenID
    #print 'curr aliq  ',current_aliquots    

def getMice(case):
    genidCase = GenealogyID(case).getGenID()
    values_to_send = {'template_id':14, 'parameters':json.dumps([{'id':'0', 'values':[genidCase]}])} # CHANGE TEMPLATE ID
    url = settings.DOMAIN_URL +  '/mdam/api/runtemplate/'
    data = urllib.urlencode(values_to_send)
    ...
    for x in result['body']:
        g = GenealogyID(x[1])# x[1] genealogy
        if len(x[13][0]) == 0: # there is no implant
            dateMouse = datetime.datetime.strptime(x[4], '%Y-%m-%d') # x[4] = 'available_date'
        else:
            dateMouse = datetime.datetime.strptime(x[13][0][0][5], '%Y-%m-%d') # x[14][0][0][5] = 'implant_date'
        print g.getGenID(), dateMouse
        nodes[g.getGenID()] = {'type':'Biomouse', 'altype': None, 'id':x[12],  'relationships':{}, 'nodeid': None, 'date':str(dateMouse)} # x[1] = id_biomouse
        buildSemanticNode(g.getGenID(), 'Biomouse', None)

        # x[14] = array of translator results (i.e. last element of the row)
        # x[14][0] = array of results for translator "Implants"
        # translator results can either include 0 rows or 1 row
        current_mice[x[1]] = [  x[13][0][0][0] if len(x[13][0]) > 0 else None] # field #0 = Aliq. GenID
...
def retrieveWG():
    q=neo4j.CypherQuery(gdb,"MATCH (n:WG) return n.identifier")
    r=q.execute()
    wgList=list()
    for data in r.data:
        if data[0]!='LASDemo_WG':
            wgList.append(data[0])
    print wgList
    return wgList
...
def resetDatabases():
    print '['+str(startTime)+'] ' + "Begin Restore DB"

    process = Popen('mysql %s -h%s -u%s -p%s' % ('lasauthserver',settings.DB_HOST, 'root','mysql2013'),stdout=PIPE, stdin=PIPE, shell=True)
    output=process.communicate('update auth_user set is_active=0;')[0]

    process = Popen('mysql %s -h%s -u%s -p%s' % ('caxeno',settings.DB_HOST, 'root','mysql2013'),stdout=PIPE, stdin=PIPE, shell=True)
    output = process.communicate('source /home/administrator/AggiornamentoXeno.sql')[0]

    process = Popen('mysql %s -h%s -u%s -p%s' % ('biobanca',settings.DB_HOST, 'root','mysql2013'),stdout=PIPE, stdin=PIPE, shell=True)
    output = process.communicate('source /home/administrator/AggiornamentoBanca.sql')[0]

    process = Popen('mysql %s -h%s -u%s -p%s' % ('storage',settings.DB_HOST, 'root','mysql2013'),stdout=PIPE, stdin=PIPE, shell=True)
    output = process.communicate('source /home/administrator/AggiornamentoStorage.sql')[0]

    process = Popen('mysql %s -h%s -u%s -p%s' % ('lasauthserver',settings.DB_HOST, 'root','mysql2013'),stdout=PIPE, stdin=PIPE, shell=True)
    output=process.communicate('update auth_user set is_active=1;')[0]
    print '['+str(startTime)+'] ' + "End Restore DB"
...
if __name__=='__main__':
    ...
    parser.add_argument('--wg', default = 'LASDemo_WG',  help='Wg for all the instances')
    args = parser.parse_args()
    startTime = datetime.datetime.now()
    print '['+str(startTime)+'] ' + "Begin Restore Graph"
    main(args)
    endTime = datetime.datetime.now()
    print 'Execution time: ',  endTime -startTime
    print '['+str(endTime)+'] ' + "End Restore Graph"
